...
-- Task 1
select  c.customer_id, c.company_name, count( distinct ord.order_id) as orders_1996,
sum(odet.unit_price * odet.quantity * (1 - odet.discount)) as total_spent_1996 -- zexchy nerarac ha
from customers c
join orders ord on c.customer_id = ord.customer_id
join order_details odet on ord.order_id = odet.order_id
where extract(year from ord.order_date) = 1996
  and not exists (
      select 1
      from orders ord2
      where ord2.customer_id = c.customer_id
        and extract(year from ord2.order_date) = 1997
  )
group by c.customer_id, c.company_name
order by total_spent_1996 desc;



-- Task 2
with total_revenue as (
    select sum(unit_price * quantity * (1 - discount)) as company_total
    from order_details
), 
category_data as (
select 
 cat.category_name,
 sum(ordet.unit_price * ordet.quantity * (1 - ordet.discount)) as category_revenue,trv.company_total
 from categories cat
 join products prod on cat.category_id = prod.category_id
 join order_details ordet on prod.product_id = ordet.product_id
 cross join total_revenue trv
 group by cat.category_name, trv.company_total
) 
select category_name,
 category_revenue,
 round((category_revenue / company_total * 100)::numeric, 2) as revenue_percentage
from category_data
where (category_revenue / company_total) >= 0.10
...
with employee_monthly_sales as (
    ...
    from employees e
    join orders ord on e.employee_id = ord.employee_id
    join order_details od on ord.order_id = od.order_id
    where extract(year from ord.order_date) = 1997
    group by e.employee_id, e.first_name, e.last_name, date_trunc('month', ord.order_date)
),
monthly_company_avg as (
    select
        ...
        ems.month,
        ems.monthly_sales,
        avg(ems.monthly_sales) over (partition by ems.month) as company_avg_sales
...
above_average_sales as (
    select
        ...
        mca.month,
        mca.monthly_sales,
        mca.company_avg_sales,
        ((mca.monthly_sales - mca.company_avg_sales) / mca.company_avg_sales) * 100 as above_avg_pct
    from monthly_company_avg mca
    where mca.monthly_sales > mca.company_avg_sales
),
best_month_per_employee as (
    select
        aas.employee_id,
        aas.full_name,
        aas.month as best_month,
        aas.monthly_sales,
        aas.company_avg_sales,
        round(aas.above_avg_pct::numeric, 2) as above_avg_pct,
        row_number() over (partition by aas.employee_id order by aas.above_avg_pct desc) as rn
    from above_average_sales aas
)
select
    bme.employee_id,
    bme.full_name,
    bme.best_month,
    bme.monthly_sales,
    bme.company_avg_sales,
    bme.above_avg_pct
from best_month_per_employee bme
where bme.rn = 1
order by bme.above_avg_pct desc;
